...
export const useColllectionsStore = create<CollectionsStore>()((set, get) => ({
  ...
  addNewCollection: (title, books) => {
    ...
    const updatedCollections = [...currentCollections, { title, books: books ?? [] }]
  ...
  deleteCollection: (title) => {
    const currentCollections = get().collections
    const updatedCollections = currentCollections.filter(c => c.title !== title)
